...
class LightningValidationPipelineTests(unittest.TestCase):
    ...
    def test_validation_step_uses_pipeline_metrics_and_writes_preview(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue((sample_dir / "target_pred_points.ply").is_file())
            self.assertTrue((sample_dir / "model_target_pred_points.ply").is_file())
            ...
            target_pred_cloud = trimesh.load(sample_dir / "target_pred_points.ply", force="mesh")
            model_target_pred_cloud = trimesh.load(sample_dir / "model_target_pred_points.ply", force="mesh")
            ...
            self.assertEqual(len(getattr(target_pred_cloud, "faces", [])), 0)
            self.assertEqual(len(getattr(model_target_pred_cloud, "faces", [])), 0)
        ...
        for artifact in artifact_logs:
            ...
            self.assertEqual(
                {name for _, name in artifact.files},
                {
                    ...
                    "target_pred_points.ply",
                    "model_target_pred_points.ply",
                },
            )

    def test_validation_step_logs_balanced_edge_statistics(self) -> None:
        module = RectangularConditionalJiTLightningModule(
            model_cfg={
                "input_size": (64, 128),
                "patch_size": 16,
        ...
        batch = {
            ...
            "model_depth": torch.randn(1, 5, 64, 128),
            "model_normal": torch.randn(1, 15, 64, 128),
            "edge_depth": torch.zeros(1, 3, 64, 128),
            "meta": [{}],
        }
        batch["edge_depth"][:, :, :16, :16] = 1.0
        ...
        module.log = capture_log
        module._trainer = SimpleNamespace(world_size=1, default_root_dir=".", global_step=0, current_epoch=0, is_global_zero=True, logger=None)

        with mock.patch("training.lightning_module.Edge3DX0BridgePipeline", _FakePipeline):
            output = module.validation_step(batch, batch_idx=0)

        self.assertIn("val/infer_balanced_l2", output)
        self.assertIn("val/infer_edge_l2", output)
        self.assertIn("val/infer_non_edge_l2", output)
        self.assertIn("val/infer_edge_pixel_fraction", output)
        self.assertIn("val/infer_non_edge_pixel_fraction", output)

        logged_names = {name for name, _ in logged}
        self.assertTrue(
            {
                "val/infer_loss_total",
                "val/infer_balanced_l2",
                "val/infer_edge_l2",
                "val/infer_non_edge_l2",
                "val/infer_edge_pixel_fraction",
                "val/infer_non_edge_pixel_fraction",
