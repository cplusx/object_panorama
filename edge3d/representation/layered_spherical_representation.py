...
@dataclass
class LayeredSphericalRepresentation:
    colors: torch.Tensor
    radii: torch.Tensor
    # inverse_radii is a derived auxiliary field.
    # The current training tensor uses raw radii/depth, not inverse radii.
...
def polylines_to_layered_spherical_representation(
    ...
    return LayeredSphericalRepresentation(
        colors=colors,
        radii=radii,
...
def _mesh_to_layered_spherical_representation_gpu_exact(
    mesh: trimesh.Trimesh,
    resolution: int,
    max_hits: int,
    outer_radius: float,
...
def mesh_to_layered_spherical_representation(
    ...
    if resolved_backend == "gpu_exact":
        return _mesh_to_layered_spherical_representation_gpu_exact(
            mesh,
...
def layered_spherical_layer_to_points(rep: LayeredSphericalRepresentation, layer: int = 0) -> torch.Tensor:
